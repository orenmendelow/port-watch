...
A macOS menu bar app that monitors active dev server ports.
...
- **Live CPU/MEM stats** -- per-process, updated every 3 seconds
...
```sh
git clone https://github.com/oren/port-watch.git
cd port-watch
./install.sh
```
...
2. Create `PortWatch.app` in `~/Applications`
...
PortWatch polls `lsof -iTCP -sTCP:LISTEN -nP` every 3 seconds on a background thread. It filters results against a known set of dev process names (node, python, ruby, go, deno, bun, vite, uvicorn, etc.) and discards system services.
